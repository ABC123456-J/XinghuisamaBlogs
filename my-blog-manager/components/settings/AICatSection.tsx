...
export default function AICatSection({ formData, handleUpdate, pushToQueue }: any) {
  // 防止 undefined
  const config = formData.chatConfig || {
  ...
  const updateConfig = (key: string, value: any) => {
    handleUpdate('chatConfig', { ...config, [key]: value });
  };
  ...
  return (
    ...
    >
      <div className="flex justify-between items-center mb-8 pb-6 border-b border-white/40 dark:border-slate-700/50">
        <div>
          <h2 className="text-2xl font-black text-slate-800 dark:text-white flex items-center gap-3 tracking-tight">
            <Bot className="text-indigo-500" size={28} /> AI 角色调度中心
          </h2>
          <p className="text-sm text-slate-500 dark:text-slate-400 font-medium mt-2 flex items-center gap-1.5">
            <Sparkles size={14} className="text-indigo-400" /> 实时调整你的 AI 角色扮演设定
          </p>
        </div>
        <button
          onClick={() => pushToQueue('AI 角色配置')}
      ...
      <div className="grid grid-cols-1 gap-8">
        ...
        <div className="group">
          <label className="flex items-center gap-2 text-sm font-black text-slate-700 dark:text-slate-300 mb-3">
            <MessageSquareText size={16} className="text-slate-400 group-focus-within:text-indigo-500 transition-colors" /> 角色 Prompt (人设)
          </label>
          <textarea
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="group">
            ...
            <input
              ...
              className="w-full h-2 bg-slate-200 dark:bg-slate-700 rounded-lg appearance-none cursor-pointer accent-indigo-500"
            />
            <p className="text-[11px] text-slate-400 mt-2">数值越大，角色说话越随机、越具创意；数值越小越严谨。</p>
          </div>
        </div>
